...
mp_size_t lmmp_random_(mp_ptr restrict dst, mp_size_t n) {
    ...
    mp_limb_t seed = lmmp_global_rng.state + mix64(n);
    seed ^= lmmp_seed_generator(n);
...
lmmp_strong_rng_t* lmmp_strong_rng_init_(mp_size_t k, unsigned seed) {
