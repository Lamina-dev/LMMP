...
void lmmp_sqr_basecase_(mp_ptr restrict dst, mp_srcptr restrict numa, mp_size_t na) {
    ...
    cl = 0;
    for (i = 1; i + 4 <= na; i += 4) {
        __uint128_t a0 = (__uint128_t)numa[i] * x + cl;
    ...
    for (j = 1; j + 1 < na; j++) {
        x = numa[j];
        cl = 0;
        for (i = j + 1; i + 4 <= na; i += 4) {
            __uint128_t a0 = (__uint128_t)numa[i] * x + dst[i + j] + cl;
            __uint128_t a1 = (__uint128_t)numa[i + 1] * x + dst[i + j + 1] + (mp_limb_t)(a0 >> 64);
            __uint128_t a2 = (__uint128_t)numa[i + 2] * x + dst[i + j + 2] + (mp_limb_t)(a1 >> 64);
            __uint128_t a3 = (__uint128_t)numa[i + 3] * x + dst[i + j + 3] + (mp_limb_t)(a2 >> 64);
            dst[i + j] = (mp_limb_t)a0;
            dst[i + j + 1] = (mp_limb_t)a1;
            dst[i + j + 2] = (mp_limb_t)a2;
            dst[i + j + 3] = (mp_limb_t)a3;
            cl = (mp_limb_t)(a3 >> 64);
        }
        for (; i < na; i++) {
            __uint128_t a = (__uint128_t)numa[i] * x + dst[i + j] + cl;
            dst[i + j] = (mp_limb_t)a;
            cl = (mp_limb_t)(a >> 64);
        }
        dst[na + j] = cl;
    ...
    cl = 0;
    for (i = 1; i + 4 <= 2 * na - 1; i += 4) {
        __uint128_t a0 = (__uint128_t)dst[i] * 2 + cl;
        __uint128_t a1 = (__uint128_t)dst[i + 1] * 2 + (mp_limb_t)(a0 >> 64);
        __uint128_t a2 = (__uint128_t)dst[i + 2] * 2 + (mp_limb_t)(a1 >> 64);
        __uint128_t a3 = (__uint128_t)dst[i + 3] * 2 + (mp_limb_t)(a2 >> 64);
        dst[i] = (mp_limb_t)a0;
        dst[i + 1] = (mp_limb_t)a1;
        dst[i + 2] = (mp_limb_t)a2;
        dst[i + 3] = (mp_limb_t)a3;
    ...
    for (; i < 2 * na - 1; i++) {
        __uint128_t a = (__uint128_t)dst[i] * 2 + cl;
        dst[i] = (mp_limb_t)a;
        cl = (mp_limb_t)(a >> 64);
    }
    dst[2 * na - 1] = cl;
