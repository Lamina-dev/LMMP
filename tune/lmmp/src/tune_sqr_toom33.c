...
/* 阈值调优：SQR_TOOM33_THRESHOLD (sqr_toom2 -> sqr_toom3)
   与乘法同构的单层 high 测量：high 仅强制根层进入 toom3。 */
...
static uint64_t get_threshold(void) { return (uint64_t)lmmp_tune_SQR_TOOM33_THRESHOLD; }
static void set_threshold(uint64_t v) { lmmp_tune_SQR_TOOM33_THRESHOLD = v; }
...
static void apply_path(uint64_t size, int use_high) {
    if (use_high)
        set_threshold(size); /* 仅强制根层进入 toom3，递归子问题仍按以下阈值回退 */
    else
...
int tune_run_sqr_toom33(void) {
    ...
    spec.macro_name = "SQR_TOOM33_THRESHOLD";
    spec.low_name = "sqr_toom2";
    spec.high_name = "sqr_toom3";
    spec.lo = lmmp_tune_SQR_TOOM22_THRESHOLD > 26 ? lmmp_tune_SQR_TOOM22_THRESHOLD : 26;
    spec.hi = lmmp_tune_SQR_TOOM44_THRESHOLD > 27 ? lmmp_tune_SQR_TOOM44_THRESHOLD - 1 : 220;
