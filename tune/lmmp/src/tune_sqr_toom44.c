...
/* 阈值调优：SQR_TOOM44_THRESHOLD (sqr_toom3 -> sqr_toom4)
   与乘法同构的单层 high 测量：high 仅强制根层进入 toom4。 */
...
static uint64_t get_threshold(void) { return (uint64_t)lmmp_tune_SQR_TOOM44_THRESHOLD; }
static void set_threshold(uint64_t v) { lmmp_tune_SQR_TOOM44_THRESHOLD = v; }
...
static void apply_path(uint64_t size, int use_high) {
    if (use_high)
        set_threshold(size); /* 仅强制根层进入 toom4，递归子问题仍按以下阈值回退 */
    else
...
int tune_run_sqr_toom44(void) {
    ...
    spec.macro_name = "SQR_TOOM44_THRESHOLD";
    spec.low_name = "sqr_toom3";
    spec.high_name = "sqr_toom4";
    spec.lo = lmmp_tune_SQR_TOOM33_THRESHOLD > 64 ? lmmp_tune_SQR_TOOM33_THRESHOLD : 64;
    spec.hi = lmmp_tune_SQR_FFT_THRESHOLD > 65 ? lmmp_tune_SQR_FFT_THRESHOLD - 1 : 1200;
