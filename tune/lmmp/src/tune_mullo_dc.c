...
static void mullo_ctx_init(mullo_ctx* c, mp_size_t n) {
    ...
    tune_fill_limbs(c->a, n, UINT64_C(0x452821e638d01377));
    tune_fill_limbs(c->b, n, UINT64_C(0xbe5466cf34e90c6c));
    c->a[n - 1] |= LIMB_B_2;
    c->b[n - 1] |= LIMB_B_2;
}
...
static void apply_path(uint64_t size, int use_high) {
    set_threshold(use_high ? 128 : size + 1);
}
